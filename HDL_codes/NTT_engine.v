module NTT_engine #(
    ...
    generate
        for (j = 0; j < STAGES-1; j = j + 1) begin : delay_gen
            Delay_units #(
                ...
                .clk(clk),
                .rst(rst),
                .start(start_delay[j]), // control signal for pipelining
    ...
    /*
    du_delay = 3, 4, 5, 6, 7 for stage 1, 2, 3, 4, 5 respectively
    du_delay_n = 3 + n for nth stage
    thus summison of du_delay for nth stage = 3 + 4 + 5 + ... + (3 + n) = 3n + (1+2+...+n) = 3n + n(n+1)/2 = n(n+5)/2
    du_delay = n(n+5)/2 (for stages 1 to n)
    bu_delay = 4 (for stages 1 to n)

    for nth butterfly unit
        delay_butterfly = 4 * (n-1) + (n-1)(n+4)/2 
    
    for nth delay unit 
        delay_du = bu_delay * n + (n-1)(n+4)/2 = 4n + (n-1)(n+4)/2
    */
    
    // ButterFly Unit Control Signals + memory Logic signal //
    genvar a;
    generate
        for (a = 1; a < STAGES+1; a = a + 1) begin : butterfly_delay_gen
            always @(*) begin
                if ((counter > (3 * (a-1) + (a-1)*(a+4)/2)) && start) begin
                    start_butterfly[a-1] <= 1; // Start butterfly after total delay
                end else begin
    ...
    generate
        for (b = 1; b < STAGES; b = b + 1) begin : delay_ctrl_gen
            always @(*) begin
                 if ((counter > 3 * b + (b-1)*(b+4)/2) && start) begin
    ...
    generate
        for (c = 2; c < STAGES+1; c = c + 1) begin : memory_ctrl_block
            always @(*) begin
                if ((counter > (3 * (c-1) + (c-1)*(c+4)/2) - 1) && start) begin
